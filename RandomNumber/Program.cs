...
namespace RandomNumberGuessingGame
{
    class Program
    {
        ...
        void Run()
        {
            ...
            Console.WriteLine("\nWelcome to Simple Version of Mastermind.");
            ...
            for (int i = 1; i <= numberOfAttempts; i++)
            {
                ...
                string digitsWrong = string.Join(",", result.Where(f => f.Flag == false)
                    .Select(c => (++c.Index).ToString()));
                ...
                else
                {
                    ...
                    digitsWrong = flagCount == 1 ? "none" : digitsWrong;
                    Console.WriteLine(string.Format("Digit(s) in place {0} is/are correct(+)", digitsCorrect));
                    Console.WriteLine(string.Format("Digit(s) in place {0} is/are not correct(-)", digitsWrong));
                }
            }
